...
tests to verify that my code could be safely merged and how to use rebasing to keep the Git history clean.
...
some of the test cases did not make any sense, yet the tests always passed. I asked my mentor about that,
...
which progressed slowly because the full CI took around 24 hours to run, depending on how many other checks
were in the queue. Therefore, any time I tried to fix something, I had to wait an entire day to see if
...
requests with more changes. I also received a new task to work on. My manager also explained the
...
was not even clear which options should be removed and which ones were actually necessary, and the program
...
so that they could review each pull request. I found out that there was a difference between an epic and a story
...
mark this epic as closed, I needed to create one last pull request to remove the old pass-disabling options.
I wrote a Python script that crawled through the codebase and found the options that were always enabled by
...
ticket. Thus, I had to create yet another pull request. It was a never-ending story. On the other hand, next
...
Tuesday of this week was St Patrick's Day, which was a bank holiday, and many co-workers took a day off on Monday
to extend their weekend. On Wednesday, I was surprised by how busy the office was and how mentally alert my
co-workers were the morning after St Patrick's Day.

Back in the office on Wednesday, my mentor explained my next ticket to me, which focused on optimising the compiler
rather than refactoring. The goal was to write an analysis tool that was able to detect which compiler passes
are outdated and do not actually modify the compiled model anymore.

== 23 to 27 March

When the last pass-disabling ticket was being merged, some merge conflicts happened, and they caused the merge
...
The next day, my mentor asked me about it, and I explained that I needed to wait for the 11,000 models to
compile. Then he explained to me that we were definitely not going to wait for 11,000 models to compile,
and that I should ask a colleague responsible for the CI which models were important for testing. I asked him,
and he told me that he would prepare the list and send it to me.

== 6 to 10 April

The colleague who promised to send me the list of models used for testing did not send me anything,
so I reminded him about that, and he told me it was in progress. I was starting to think that if I hadn't
bothered asking him and had simply run the analysis on all models instead, it would have been finished by
then. In the meantime, I wrote documentation for the new pass-disabling mechanism and merged it into
the repository.

On Thursday, my manager asked me how my internship was going and whether I would consider working at
Intel after graduation. I replied that I thought my internship was going well and that I would be happy to
work there.
...
are repeated multiple times (e.g. Convolution, Clamp, BatchNorm). At the time, the compiler had to
do all compilation steps for each repetition separately, which could take a long time for large
models. If we compiled such a repeating block only once and then called the compiled block repeatedly,
the compilation could get much faster.

The first ticket pertaining to this problem was adding support for repeating blocks in a compiler
...
the pass. After that, my pull request got merged and the ticket was closed.
...
slowing down our tests. Every test was evaluating that operation even though most tests did not need
its result. Lazy evaluation ensured we did not evaluate it unless we needed it, making our test suite
run three times faster.

== 27 April to 1 May

I got both tickets from last week merged into the upstream. Then I finally tested my outdated
pass detection tool on around 500 models for the Intel Panther Lake architecture. When I got the
...
extract them all into subroutines. The pass used to extract only repeating blocks, so my task
...
manually in the CI to get results for more models than the 500 I had used for testing before, and for other
architectures than just Panther Lake. Still, the result was that 62 passes were never used for any of
...
one of the senior programmers doing code review said it conflicted with a task another programmer was
working on. We scheduled a call to figure it out. The issue was that there was an MLIR instrumentation to
...
Fortunately, there was a class in MLIR for this purpose that allowed registering multiple observers
...
Thursdays, where we discuss progress on tasks during the past week and plans for the next week.

For instant communication, we rely on Microsoft Teams. Given that most employees come to the office on some
workdays, it is also often possible to discuss things in person.
...
Every pull request requires approval from two code reviewers and must pass the CI (continuous integration)
checks. After that, a maintainer adds an auto-merge label to the pull request and an automated Jenkins job
...
the upstream in order to pre-empt the possibility that two changes from two different pull requests interact
